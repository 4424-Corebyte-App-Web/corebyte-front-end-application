...
export default {
  ...
  methods: {
    cancel() {
      this.$router.push({ name: "PressingStage" });
    },
    ...
    async saveBatch() {
      ...
      try {
        ...
        // Generate a new GUID for the batch name
        const generateGuid = () => {
        ...
        const batchData = {
          name: generateGuid(), // Use GUID for name
          type: this.batch.type,
          status: this.batch.status,
          temperature: parseFloat(this.batch.temperature), // Convert to number, backend expects double
          amount: this.batch.amount,
          total: parseFloat(this.batch.total) || 0, // Convert to number, backend expects decimal
          date: new Date(this.batch.date).toISOString(), // Ensure proper date format
          nLote: this.batch.NLote.replace('#', '') // Remove # from NLote if present
        };
        ...
        let errorMessage = "No se pudo guardar el lote. Por favor, intente nuevamente.";
...
<template>
  <div class="form-container">
    <div class="form-header">
      <h1>Nuevo registro de prensado</h1>
      <pv-button
