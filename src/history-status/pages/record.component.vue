...
export default {
  ...
  computed: {
    filteredProducts() {
      if (!this.value) return this.records;
  ...
  methods: {
    search(event) {
      ...
      this.items = [...new Set(this.records
    ...
    generatePdfReport() {
      try {
        ...
        const tableColumn = ["ID", "Customer", "Type", "Year", "Product", "Batch", "Stock"];
        const tableRows = [];
        ...
        <div class="table-and-button-container">
          <div class="data-table" style="flex-grow: 1;">
            <pv-datatable :value="filteredProducts" paginator :rows="13" :rowsPerPageOptions="[5, 10, 20, 50]" tableStyle="min-width: 60rem">
              ...
              <pv-column field="product" header="Product" sortable style="width: 10%"></pv-column>
